...
async function executeClaudeMasterTurn(params: {
  ...
  const { context, sessionHints, sessionId, invokeTool, allowedToolNames } = params
  ...
  for await (const message of stream) {
    ...
    if (message.type === 'result') {
      if (message.subtype === 'success') {
        ...
        const errorText = (message as any).errors?.join('; ') || 'Unknown Claude Agent SDK error'
...
function buildMasterSdkTools(
  ...
  return [
    tool('get_master_snapshot', 'Get current master runtime snapshot.', {}, async () =>
      asToolResult(await invokeTool('get_master_snapshot')),
    ),
    tool(
      'list_tasks',
    ...
    tool('get_task', 'Get a single task by id.', { taskId: z.string() }, async (args) =>
      asToolResult(await invokeTool('get_task', args)),
    ),
    tool(
      'get_recent_history',
      'Get recent history entries.',
      { limit: z.number().int().positive().optional() },
      async (args) => asToolResult(await invokeTool('get_recent_history', args)),
    ),
    tool(
      'get_task_diff',
    ...
    tool('cancel_task', 'Cancel a task.', { taskId: z.string() }, async (args) =>
      asToolResult(await invokeTool('cancel_task', args)),
    ),
    tool(
      'retry_task',
    ...
    tool(
      ...
      { taskId: z.string() },
      async (args) => asToolResult(await invokeTool('cleanup_task_artifacts', args)),
    ),
    tool(
      'ask_human',
